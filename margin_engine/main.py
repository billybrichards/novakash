...
async def run() -> None:
    ...
    # Strategy decision recorder — writes per-strategy evaluations to strategy_decisions
    # for backtesting and post-trade analysis.
    from margin_engine.adapters.persistence.pg_strategy_decision_repository import (
        PgStrategyDecisionRepository,
        AsyncStrategyDecisionRecorder,
    )

    strategy_decision_repo = PgStrategyDecisionRepository(pool)
    await strategy_decision_repo.ensure_table()
    strategy_decision_recorder = AsyncStrategyDecisionRecorder(
        strategy_decision_repo, asyncio.get_running_loop()
    )
    strategy_decision_recorder.start()
    ...
    open_uc = OpenPositionUseCase(
        ...
        strategy_decision_recorder=strategy_decision_recorder,
        # ── v4 integration (PR B) ──
        ...
        v4_fee_wall_continuation_divisor=settings.v4_fee_wall_continuation_divisor,
        fee_aware_continuation_enabled=settings.fee_aware_continuation_enabled,
    ...
    try:
        ...
        await (
            strategy_decision_recorder.stop()
        )  # flush remaining decisions before pool closes
