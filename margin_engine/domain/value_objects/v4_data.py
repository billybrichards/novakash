"""
Domain value objects for v4 snapshot data.

These dataclasses represent domain concepts (regime, macro bias, consensus)
that are central to the margin engine's business logic. They are moved here
from adapters/signal/v4_models.py to maintain clean architecture - the domain
layer should not depend on adapter layer.

The adapter layer now imports from here and handles HTTP parsing → domain
object conversion.
"""
...
if TYPE_CHECKING:
    ...
    # Lazy import at runtime to avoid circular import
    TradeSide = None
