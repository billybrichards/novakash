...
const SERVICES = [
  {
    ...
    branch: 'main',
    host: '3.98.114.0',
    hostLabel: 'EC2 Montreal (ca-central-1, shared with macro-observer + data-collector)',
    ...
    notes: 'Primary model service. v2 (LightGBM + quantiles), v3 (composite multiscale), v4 (fusion surface). Port 8080. All endpoints proxied through hub/api/margin.py.',
  },
  {
    id: 'macro-observer',
    name: 'macro-observer',
    ...
    branch: 'develop',
    host: '3.98.114.0',
    hostLabel: 'EC2 Montreal (same box as timesfm + data-collector)',
    ...
    secretsNeeded: ['MACRO_OBSERVER_HOST', 'MACRO_OBSERVER_SSH_KEY', 'DATABASE_URL', 'QWEN_API_KEY', 'QWEN_BASE_URL', 'TELEGRAM_BOT_TOKEN', 'TELEGRAM_CHAT_ID'],
    notes: 'Runs in Docker on the shared Montreal box. Writes to macro_signals table every 60s. Qwen 3.5 swapped in via PR #15 (previously Anthropic). Per-timescale bias added via PR #17. Consumed by /v4/macro in timesfm-service.',
  },
  {
    id: 'data-collector',
    name: 'data-collector',
    ...
    branch: 'develop',
    host: '3.98.114.0',
    hostLabel: 'EC2 Montreal (same box as timesfm + macro-observer)',
    workflow: '.github/workflows/deploy-data-collector.yml',
    workflowStatus: 'active',
    deployTriggers: 'push to develop, path data-collector/**',
    healthProbePath: null,
    secretsNeeded: ['MACRO_OBSERVER_HOST', 'MACRO_OBSERVER_SSH_KEY', 'DATABASE_URL'],
    notes: 'Writes to market_data + market_snapshots tables (~2.3/sec, 8 writes per ~3.5s Gamma-API-bound cycle). Migrated from Railway to AWS Montreal in PR #24. Docker container with healthcheck on /tmp/collector.alive (30s window).',
  },
  {
    id: 'margin-engine',
    name: 'margin-engine',
  ...
  },
  {
    ...
    repo: 'novakash',
    branch: 'develop',
    host: 'hub-develop-0433.up.railway.app',
    hostLabel: 'Railway — still on Railway, TODO migrate to AWS',
    workflow: 'Railway auto-deploy from develop (no GH Actions workflow)',
    workflowStatus: 'legacy',
    deployTriggers: 'push to develop (Railway git-watcher)',
    healthProbePath: '/api/system/status',
    healthProbeVia: 'api',
    secretsNeeded: ['DATABASE_URL (Railway project env)', 'Railway project secrets'],
    notes: 'Proxies to timesfm + margin-engine + engine via httpx. Handles auth (JWT) and all REST+WS endpoints the frontend talks to. A 401 on /api/* is healthy (auth wall present). Next migration candidate after engine.',
  },
  {
    id: 'frontend',
    name: 'frontend (web)',
    ...
    notes: 'Vite + React 18. Deploys via npm ci + npm run build + tar + scp + systemctl restart nginx. Proxies /api/* to hub (Railway) and /auth/* to hub auth. Nginx upstream still references hub-develop-0433.up.railway.app (will need update when hub migrates).',
  },
  {
    id: 'engine',
