...
const TASKS = [
  ...
  },
  {
    ...
    evidence: [
      'User request 2026-04-11: "add a frontend page called notes and have it as a db table of notes and things observations etc as we work". Add as the FIRST entry in the sidebar (top of the nav) so it\'s always reachable as Claude + user are working through the audit.',
      'Should be DB-backed (not static) so notes persist across frontend redeploys and can be added/edited/deleted at runtime.',
      'Usage: Claude will post notes to the page as observations accumulate during the session. Think of it as a session journal that the /audit page references for deeper context.',
      'Needs: SQLAlchemy model in hub/db/models.py, FastAPI CRUD routes in hub/api/notes.py (proxy to shared Postgres), frontend page with list + add/edit/delete, sidebar nav entry at position 0.',
    ],
    fix: 'Dispatching background Agent F (NT-01) in an isolated worktree to build the whole stack: DB migration (CREATE TABLE notes), SQLAlchemy model, FastAPI router, useApi frontend page with form + list view + optimistic updates. The agent will target develop on novakash and open a PR. Status IN_PROGRESS until the agent returns.',
    progressNotes: [
      { date: '2026-04-11', note: 'Dispatched as Agent F after user request during the post-INC-01 session. Will be the first entry in the sidebar once merged.' },
    ],
  },
  {
    id: 'UI-01',
