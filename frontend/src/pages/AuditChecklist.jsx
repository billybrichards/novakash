...
const TASKS = [
  ...
  },
  {
    ...
    severity: 'HIGH',
    status: 'OPEN',
    title: 'Live trade panel is broken end-to-end — investigate + fix',
    files: [
      ...
      { path: 'engine/persistence/db_client.py', line: 1101, repo: 'novakash' },
    ],
    evidence: [
      'User report 2026-04-11: "the live trade path doesnt seem to currently work i tried it".',
      'Code audit confirms the FULL flow exists in the codebase: ManualTradePanel.jsx posts to /api/v58/manual-trade → hub writes to manual_trades table with status=pending_live → engine polls poll_pending_live_trades() via orchestrator.py:2515 → executes via polymarket_client.',
      'User tried the panel and nothing happened. So SOMEWHERE in the pipeline the flow is broken. Candidates: (a) hub endpoint returns error to frontend but UI doesn\'t surface it clearly, (b) DB write succeeds but engine polling loop is not running, (c) engine polls but fails to execute the trade, (d) trade executes but no Telegram notification fires so the user thinks nothing happened.',
      'Montreal rules: the actual Polymarket API call MUST happen from the Montreal engine (not the hub) because the Polymarket API geo-blocks non-Montreal IPs. The current architecture is correct — hub writes to DB, engine on Montreal polls + executes. So "broken" probably means a step in that chain is silently failing.',
    ],
    fix: 'Phase 0 investigation: (1) POST /api/v58/manual-trade via curl from main session with a small paper stake, check response. (2) SELECT from manual_trades WHERE trade_id=<new> — did it land in DB? (3) Tail engine.log for orchestrator.pending_trades_poll — is the poll loop running? (4) SELECT from manual_trades ORDER BY created_at DESC LIMIT 5 — are there historical pending_live rows stuck? (5) If stuck, identify the failing step and add a progressNote.',
  },
  {
    id: 'LT-03',
    category: 'frontend',
