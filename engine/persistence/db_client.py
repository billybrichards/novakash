...
class DBClient:
            ...
            async with self._pool.acquire() as conn:
                await conn.execute(
                    ...
                    ON CONFLICT (window_ts, asset, timeframe, COALESCE(eval_offset, -1)) DO UPDATE SET
                        ...
                        probability_lgb_v9_3_btc_pure = COALESCE(EXCLUDED.probability_lgb_v9_3_btc_pure, window_snapshots.probability_lgb_v9_3_btc_pure),
                        probability_lgb_v9_2_pure = COALESCE(EXCLUDED.probability_lgb_v9_2_pure, window_snapshots.probability_lgb_v9_2_pure),
                        probability_lgb_v12_pure = COALESCE(EXCLUDED.probability_lgb_v12_pure, window_snapshots.probability_lgb_v12_pure),
                        probability_lgb_v9_5_xrp = COALESCE(EXCLUDED.probability_lgb_v9_5_xrp, window_snapshots.probability_lgb_v9_5_xrp),
                        probability_v2_meta_gate   = COALESCE(EXCLUDED.probability_v2_meta_gate, window_snapshots.probability_v2_meta_gate),
                        probability_v9_2_meta_gate  = COALESCE(EXCLUDED.probability_v9_2_meta_gate, window_snapshots.probability_v9_2_meta_gate),
                        probability_v12_meta_gate   = COALESCE(EXCLUDED.probability_v12_meta_gate, window_snapshots.probability_v12_meta_gate)
                    """,
                    ...
                    ensemble_fields.get("probability_lgb_v9_3_btc_pure"),
                    ensemble_fields.get("probability_lgb_v9_2_pure"),
                    ensemble_fields.get("probability_lgb_v12_pure"),
                    ensemble_fields.get("probability_lgb_v9_5_xrp"),
    ...
    async def update_signal_evaluations_lgb_v9_2_eth(
        self,
        window_ts,
        asset: str,
        timeframe: str,
        eval_offset: Optional[int],
        probability_lgb_v9_2_eth: Optional[float],
    ) -> int:
        """Upsert ``probability_lgb_v9_2_eth`` onto signal_evaluations row.

        Mirror of update_signal_evaluations_lgb_v9_2_post_iso for the
        ETH-trained v9.2-style LGB head. Column added by
        migrations/add_probability_lgb_v9_2_eth.sql (2026-05-20).

        The probability is emitted by timesfm-service when V9_2_ETH_ENABLED=true
        (companion bg-agent-1 PR). Read by the v9_2_eth_raw_lgb GHOST strategy.

        Idempotent: COALESCE preserves any existing value on conflict
        (first-write-wins, mirrors the v12 / v9_1 / post_iso writer parity).
        Returns row count affected (1 = upsert ok, 0 = no-op).

        Hub notes #545 (data inventory), #547 (pipeline), #550 (training results).
        """
        if not self._pool:
            return 0
        if probability_lgb_v9_2_eth is None:
            return 0
        if eval_offset is None:
            return 0
        try:
            async with self._pool.acquire(timeout=5) as conn:
                result = await conn.execute(
                    ...
                    INSERT INTO signal_evaluations (
                        window_ts, asset, timeframe, eval_offset,
                        probability_lgb_v9_2_eth, evaluated_at
                    ) VALUES (
                        $1, $2, $3, $4, $5, NOW()
                    )
                    ON CONFLICT (window_ts, asset, timeframe, eval_offset) DO UPDATE SET
                        -- First-write-wins (mirrors v12 / v9_1 / post_iso upsert
                        -- behaviour). The ETH probability is computed once per
                        -- tick by the timesfm scorer and is stable for that
                        -- (window, eval_offset). If multiple writers attempt
                        -- the same upsert (registry trace + ensemble surface),
                        -- the first non-NULL wins and subsequent attempts are
                        -- no-ops. evaluated_at is NOT refreshed on conflict —
                        -- intentional parity with sibling writers.
                        probability_lgb_v9_2_eth = COALESCE(
                            signal_evaluations.probability_lgb_v9_2_eth,
                            EXCLUDED.probability_lgb_v9_2_eth
                        )
                    """,
                    int(window_ts),
                    asset,
                    timeframe,
                    int(eval_offset),
                    float(probability_lgb_v9_2_eth),
                )
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "db.signal_evaluations_lgb_v9_2_eth_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
    ...
    async def update_signal_evaluations_lgb_v9_5_eth(
        self,
        window_ts,
        asset: str,
        timeframe: str,
        eval_offset: Optional[int],
        probability_lgb_v9_5_eth: Optional[float],
    ) -> int:
        """Upsert ``probability_lgb_v9_5_eth`` onto signal_evaluations row.

        Sibling of update_signal_evaluations_lgb_v9_2_eth for the v9.5
        ETH-trained LGB head. Column added by
        migrations/add_probability_lgb_v9_5_eth.sql (2026-05-22).
        ...
        writer parity). Returns row count affected (1 = upsert ok, 0 = no-op).

        RDS notes #579 (dedup analysis), #584 (final operating point).
        """
        if not self._pool:
            return 0
        if probability_lgb_v9_5_eth is None:
            return 0
        if eval_offset is None:
            return 0
        try:
            async with self._pool.acquire(timeout=5) as conn:
                result = await conn.execute(
                    ...
                    INSERT INTO signal_evaluations (
                        window_ts, asset, timeframe, eval_offset,
                        probability_lgb_v9_5_eth, evaluated_at
                    ) VALUES (
                        $1, $2, $3, $4, $5, NOW()
                    )
                    ON CONFLICT (window_ts, asset, timeframe, eval_offset) DO UPDATE SET
                        -- First-write-wins (mirrors sibling writers). The v9.5
                        -- probability is computed once per tick by the timesfm
                        -- scorer and is stable for that (window, eval_offset).
                        probability_lgb_v9_5_eth = COALESCE(
                            signal_evaluations.probability_lgb_v9_5_eth,
                            EXCLUDED.probability_lgb_v9_5_eth
                        )
                    """,
                    int(window_ts),
                    asset,
                    timeframe,
                    int(eval_offset),
                    float(probability_lgb_v9_5_eth),
                )
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "db.signal_evaluations_lgb_v9_5_eth_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
    ...
    async def update_signal_evaluations_lgb_v9_3_btc(
        ...
        """Upsert ``probability_lgb_v9_3_btc`` onto signal_evaluations row.

        Sibling of update_signal_evaluations_lgb_v9_2_eth for the v9.3
        BTC-trained LGB head. Column added by
        migrations/add_probability_lgb_v9_3_btc_column.sql (2026-05-22).
        ...
        try:
            async with self._pool.acquire(timeout=5) as conn:
                result = await conn.execute(
                    ...
                    ON CONFLICT (window_ts, asset, timeframe, eval_offset) DO UPDATE SET
                        -- First-write-wins (mirrors sibling writers). The v9.3
                        -- BTC probability is computed once per tick by the
                        -- timesfm scorer and is stable for that (window,
                        -- eval_offset).
            ...
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "db.signal_evaluations_lgb_v9_3_btc_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
    ...
    async def update_signal_evaluations_lgb_v9_3_btc_pure(
        self,
        window_ts,
        asset: str,
        timeframe: str,
        eval_offset: Optional[int],
        probability_lgb_v9_3_btc_pure: Optional[float],
    ) -> int:
        """Upsert ``probability_lgb_v9_3_btc_pure`` onto signal_evaluations row.

        Sibling of update_signal_evaluations_lgb_v9_3_btc — PURE persists the
        un-blended LGB+iso output (blend caps at 0.916 vs pure max 1.000 due
        to TimesFM HF classifier saturation, RDS notes #618/#631/#632).
        Column added by migrations/add_probability_lgb_btc_pure_columns.sql
        (2026-05-24).

        Read by the v9_3_btc_pure_lgb GHOST strategy (this PR).

        Idempotent: COALESCE preserves any existing value on conflict
        (first-write-wins, mirrors sibling writers). Returns row count
        affected (1 = upsert ok, 0 = no-op).

        Walk-forward CV: /tmp/btc_walkforward_results.md (v9.3 PURE
        1725 fires @ 90.3% WR — best stand-alone BTC head, ~4x more fires
        than blend at the same WR target).
        """
        if not self._pool:
            return 0
        if probability_lgb_v9_3_btc_pure is None:
            return 0
        if eval_offset is None:
            return 0
        try:
            async with self._pool.acquire(timeout=5) as conn:
                result = await conn.execute(
                    ...
                    INSERT INTO signal_evaluations (
                        window_ts, asset, timeframe, eval_offset,
                        probability_lgb_v9_3_btc_pure, evaluated_at
                    ) VALUES (
                        $1, $2, $3, $4, $5, NOW()
                    )
                    ON CONFLICT (window_ts, asset, timeframe, eval_offset) DO UPDATE SET
                        probability_lgb_v9_3_btc_pure = COALESCE(
                            signal_evaluations.probability_lgb_v9_3_btc_pure,
                            EXCLUDED.probability_lgb_v9_3_btc_pure
                        )
                    """,
                    int(window_ts),
                    asset,
                    timeframe,
                    int(eval_offset),
                    float(probability_lgb_v9_3_btc_pure),
                )
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "db.signal_evaluations_lgb_v9_3_btc_pure_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
    ...
    async def update_signal_evaluations_tickformer_v16(
        ...
        """Upsert ``probability_tickformer_v16`` onto signal_evaluations row.

        Written by the tickformer_v16_pure SHADOW strategy sidecar (PR #619).
        Idempotent: COALESCE preserves existing value on conflict.
        """
        if not self._pool:
            return 0
        ...
        try:
            ...
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "db.signal_evaluations_tickformer_v16_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
            ...
            log.warning(
                "db.update_signal_evaluations_tickformer_v16_failed",
                window_ts=window_ts,
                asset=asset,
                **exc_log_fields(exc, max_len=160),
            )
    ...
    async def update_signal_evaluations_tickformer_v17(
        ...
        """Upsert ``probability_tickformer_v17`` onto signal_evaluations row.
        ...
        try:
            ...
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "db.signal_evaluations_tickformer_v17_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
            ...
            log.warning(
                "db.update_signal_evaluations_tickformer_v17_failed",
                window_ts=window_ts,
                asset=asset,
                **exc_log_fields(exc, max_len=160),
            )
    ...
    async def update_signal_evaluations_tickformer_v18(
        ...
        """Upsert ``probability_tickformer_v18`` onto signal_evaluations row.
        ...
        try:
            ...
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "db.signal_evaluations_tickformer_v18_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
            ...
            log.warning(
                "db.update_signal_evaluations_tickformer_v18_failed",
                window_ts=window_ts,
                asset=asset,
                **exc_log_fields(exc, max_len=160),
            )
    ...
    async def update_signal_evaluations_lgb_v9_2_pure(
        ...
        timeframe: str,
        eval_offset: Optional[int],
        probability_lgb_v9_2_pure: Optional[float],
    ) -> int:
        """Upsert ``probability_lgb_v9_2_pure`` onto signal_evaluations row.

        Sibling of update_signal_evaluations_lgb_v9_3_btc_pure — PURE persists
        the v9.2 BTC un-blended LGB+iso output (blend caps at 0.826 vs pure
        max 0.956, RDS notes #618/#631/#632). Column added by
        migrations/add_probability_lgb_btc_pure_columns.sql (2026-05-24).

        Read by the v9_2_v12_combo_pure GHOST strategy (this PR).
        ...
        if not self._pool:
            return 0
        if probability_lgb_v9_2_pure is None:
            return 0
        if eval_offset is None:
            return 0
        try:
            async with self._pool.acquire(timeout=5) as conn:
                result = await conn.execute(
                    ...
                    INSERT INTO signal_evaluations (
                        window_ts, asset, timeframe, eval_offset,
                        probability_lgb_v9_2_pure, evaluated_at
                    ) VALUES (
                        $1, $2, $3, $4, $5, NOW()
                    )
                    ON CONFLICT (window_ts, asset, timeframe, eval_offset) DO UPDATE SET
                        probability_lgb_v9_2_pure = COALESCE(
                            signal_evaluations.probability_lgb_v9_2_pure,
                            EXCLUDED.probability_lgb_v9_2_pure
                        )
                    """,
                    int(window_ts),
                    asset,
                    timeframe,
                    int(eval_offset),
                    float(probability_lgb_v9_2_pure),
                )
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "db.signal_evaluations_lgb_v9_2_pure_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
    ...
    async def update_signal_evaluations_lgb_v12_pure(
        self,
        window_ts,
        asset: str,
        timeframe: str,
        eval_offset: Optional[int],
        probability_lgb_v12_pure: Optional[float],
    ) -> int:
        """Upsert ``probability_lgb_v12_pure`` onto signal_evaluations row.

        Sibling of update_signal_evaluations_lgb_v9_2_pure — PURE persists
        the v12 BTC un-blended LGB+iso output (blend caps at 0.805 vs pure
        max 0.921, RDS notes #618/#631/#632). Column added by
        migrations/add_probability_lgb_btc_pure_columns.sql (2026-05-24).
        ...
        if not self._pool:
            return 0
        if probability_lgb_v12_pure is None:
            return 0
        if eval_offset is None:
            return 0
        try:
            async with self._pool.acquire(timeout=5) as conn:
                result = await conn.execute(
                    ...
                    INSERT INTO signal_evaluations (
                        window_ts, asset, timeframe, eval_offset,
                        probability_lgb_v12_pure, evaluated_at
                    ) VALUES (
                        $1, $2, $3, $4, $5, NOW()
                    )
                    ON CONFLICT (window_ts, asset, timeframe, eval_offset) DO UPDATE SET
                        probability_lgb_v12_pure = COALESCE(
                            signal_evaluations.probability_lgb_v12_pure,
                            EXCLUDED.probability_lgb_v12_pure
                        )
                    """,
                    int(window_ts),
                    asset,
                    timeframe,
                    int(eval_offset),
                    float(probability_lgb_v12_pure),
                )
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "db.signal_evaluations_lgb_v12_pure_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
    ...
    async def update_signal_evaluations_lgb_v9_5_xrp(
        ...
        timeframe: str,
        eval_offset: Optional[int],
        probability_lgb_v9_5_xrp: Optional[float],
    ) -> int:
        """Upsert ``probability_lgb_v9_5_xrp`` onto signal_evaluations row.

        Sibling of update_signal_evaluations_lgb_v9_3_btc for the v9.5
        XRP-trained LGB head. Column added by
        ...
        Idempotent: COALESCE preserves any existing value on conflict
        (first-write-wins, mirrors the v12 / v9_1 / post_iso / v9_2_eth /
        v9_3_btc writer parity). Returns row count affected (1 = upsert ok,
        0 = no-op).

        Timesfm-repo PR #160 + RDS note #593 (walk-forward CV: raw_lgb 74.8%
        UP / 71.9% DOWN; tight 80.9% UP / 83.1% DOWN).
        """
        if not self._pool:
            return 0
        if probability_lgb_v9_5_xrp is None:
            return 0
        if eval_offset is None:
            return 0
        try:
            async with self._pool.acquire(timeout=5) as conn:
                result = await conn.execute(
                    ...
                    INSERT INTO signal_evaluations (
                        window_ts, asset, timeframe, eval_offset,
                        probability_lgb_v9_5_xrp, evaluated_at
                    ) VALUES (
                        $1, $2, $3, $4, $5, NOW()
                    )
                    ON CONFLICT (window_ts, asset, timeframe, eval_offset) DO UPDATE SET
                        -- First-write-wins (mirrors sibling writers). The v9.5
                        -- XRP probability is computed once per tick by the
                        -- timesfm scorer and is stable for that (window,
                        -- eval_offset).
                        probability_lgb_v9_5_xrp = COALESCE(
                            signal_evaluations.probability_lgb_v9_5_xrp,
                            EXCLUDED.probability_lgb_v9_5_xrp
                        )
                    """,
                    int(window_ts),
                    asset,
                    timeframe,
                    int(eval_offset),
                    float(probability_lgb_v9_5_xrp),
                )
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "db.signal_evaluations_lgb_v9_5_xrp_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
    ...
    async def update_signal_evaluations_tickformer(
        ...
        Writes probability_tickformer_v16/v17/v18/v20 (NUMERIC) plus the two
        auxiliary fields emitted by timesfm-service:
        ...
        if eval_offset is None:
            return 0
        # Only skip entirely when ALL probe columns are None.
        if all(
            v is None
            for v in (
        ...
        try:
            ...
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "db.signal_evaluations_tickformer_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
                has_v16=probability_tickformer_v16 is not None,
                has_v17=probability_tickformer_v17 is not None,
                has_v18=probability_tickformer_v18 is not None,
            ...
            log.warning(
                "db.update_signal_evaluations_tickformer_failed",
                window_ts=window_ts,
                asset=asset,
                **exc_log_fields(exc, max_len=160),
            )
    ...
    async def update_signal_evaluations_tickformer_eth_xrp(
        ...
        Mirror of update_signal_evaluations_tickformer for the asset-suffixed
        heads emitted by the classifier-gpu sister PR (feat/v4-eth-xrp-emit).
        These columns were added to signal_evaluations by
        migrations/add_tickformer_eth_xrp_columns.sql (RDS note #833).

        COALESCE semantics: first-write-wins on all 8 columns, matching every
        other sibling writer in this class.

        Skipped entirely when eval_offset is None (no stable row key) or when
        ALL 8 columns are None (nothing to write).
        ...
        try:
            async with self._pool.acquire(timeout=5) as conn:
                result = await conn.execute(
                    ...
                    ON CONFLICT (window_ts, asset, timeframe, eval_offset) DO UPDATE SET
                        -- First-write-wins (COALESCE) for all 8 columns.
            ...
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "db.signal_evaluations_tickformer_eth_xrp_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
                has_v16_eth=probability_tickformer_v16_eth is not None,
                has_v17_eth=probability_tickformer_v17_eth is not None,
                has_v18_eth=probability_tickformer_v18_eth is not None,
                has_v20_eth=probability_tickformer_v20_eth is not None,
                has_v16_xrp=probability_tickformer_v16_xrp is not None,
                has_v17_xrp=probability_tickformer_v17_xrp is not None,
                has_v18_xrp=probability_tickformer_v18_xrp is not None,
                has_v20_xrp=probability_tickformer_v20_xrp is not None,
                rows=n,
            )
            return n
        except Exception as exc:
            log.warning(
                "db.update_signal_evaluations_tickformer_eth_xrp_failed",
                window_ts=window_ts,
                asset=asset,
                **exc_log_fields(exc, max_len=160),
            )
