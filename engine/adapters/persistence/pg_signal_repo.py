...
class PgSignalRepository(SignalRepository):
    ...
    async def update_window_ensemble_fields(
        ...
        *,
        ...
        StrategyRegistry._write_window_trace so historical counterfactual
        WR (p_lgb alone vs p_classifier alone vs ensemble p_up) is
        queryable in SQL. See migration
        ...
        Verbatim SQL from DBClient.update_window_ensemble_fields.
        """
        if not self._pool:
            return
        if eval_offset is None:
            ...
            async with self._pool.acquire() as conn:
                await conn.execute(
                    ...
                    ON CONFLICT (window_ts, asset, timeframe, COALESCE(eval_offset, -1)) DO UPDATE SET
                        ...
                        probability_lgb_v9_5_eth_pure = COALESCE(EXCLUDED.probability_lgb_v9_5_eth_pure, window_snapshots.probability_lgb_v9_5_eth_pure),
                    ...
                    ensemble_fields.get("probability_lgb_v9_5_eth_pure"),
    ...
    async def update_signal_evaluations_lgb_v12(
        ...
        """Upsert ``probability_lgb_v12`` on signal_evaluations row.

        Mirror of DBClient.update_signal_evaluations_lgb_v12. Audit-task
        #332 — column existed (migration applied) but the original
        UPDATE-only writer no-op'd ~99.99% of the time because the
        canonical ``write_signal_evaluation`` INSERT typically had not
        yet fired for this exact (window_ts, asset, timeframe,
        eval_offset). This implementation upserts a minimal row when
        absent and lets the canonical writer's later UPSERT fill rich
        columns (which uses raw EXCLUDED.* assignment so it does not
        clobber our v12 stamp — verified in db_client.py).
        ...
        try:
            ...
            log.debug(
                "pg_signal_repo.signal_evaluations_lgb_v12_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
    ...
    async def update_signal_evaluations_lgb_v9_2(
        ...
        """Upsert v9.2-optuna fields on signal_evaluations row.

        Mirror of DBClient.update_signal_evaluations_lgb_v9_2 (verbatim
        parity so the two writers cannot drift — lesson from PR #439).

        ``v9_2_gate_fired`` uses OR-merge (TRUE wins) so a TRADE-tick
        stamp wins over earlier SKIP-tick stamps within the same row.

        Idempotent for the COALESCE-protected fields.
        """
        if not self._pool:
            return 0
        ...
        try:
            ...
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "pg_signal_repo.signal_evaluations_lgb_v9_2_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
    ...
    async def update_signal_evaluations_lgb_v9_2_eth(
        ...
        """Upsert ``probability_lgb_v9_2_eth`` on signal_evaluations row.

        Mirror of DBClient.update_signal_evaluations_lgb_v9_2_eth for parity
        (lesson from PR #439 — keep the two writers verbatim). Column added
        by migrations/add_probability_lgb_v9_2_eth.sql (2026-05-20).

        Hub notes #545 / #547 / #550. Idempotent via COALESCE.
        """
        if not self._pool:
            return 0
        ...
        try:
            ...
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "pg_signal_repo.signal_evaluations_lgb_v9_2_eth_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
    ...
    async def update_signal_evaluations_lgb_v9_5_eth(
        ...
        """Upsert ``probability_lgb_v9_5_eth`` on signal_evaluations row.

        Mirror of DBClient.update_signal_evaluations_lgb_v9_5_eth for parity
        (lesson from PR #439 — keep the two writers verbatim). Column added
        by migrations/add_probability_lgb_v9_5_eth.sql (2026-05-22).

        Sibling of update_signal_evaluations_lgb_v9_2_eth. RDS notes #579/#584.
        Idempotent via COALESCE.
        """
        if not self._pool:
            return 0
        ...
        try:
            ...
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "pg_signal_repo.signal_evaluations_lgb_v9_5_eth_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
    ...
    async def update_signal_evaluations_lgb_v9_5_eth_pure(
        ...
        """Upsert ``probability_lgb_v9_5_eth_pure`` on signal_evaluations row.

        Sibling of update_signal_evaluations_lgb_v9_5_eth — PURE persists the
        un-blended LGB+iso output (the LIVE blend caps at ~0.92 because the
        TimesFM HF classifier saturates at ~0.84, RDS notes #631/#632).
        Column added by migrations/add_probability_lgb_v9_5_eth_pure.sql
        (2026-05-24). Idempotent via COALESCE.
        """
        if not self._pool:
            return 0
        ...
        try:
            ...
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "pg_signal_repo.signal_evaluations_lgb_v9_5_eth_pure_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
    ...
    async def update_signal_evaluations_lgb_v9_3_btc_pure(
        ...
        """Upsert ``probability_lgb_v9_3_btc_pure`` on signal_evaluations row.

        Sibling of update_signal_evaluations_lgb_v9_3_btc — PURE persists the
        un-blended LGB+iso output (blend caps at 0.916 due to TimesFM HF
        classifier saturation, RDS notes #618/#631/#632). Column added by
        migrations/add_probability_lgb_btc_pure_columns.sql (2026-05-24).
        Read by the v9_3_btc_pure_lgb GHOST strategy. Idempotent via COALESCE.
        Walk-forward CV: /tmp/btc_walkforward_results.md (v9.3 PURE 1725 fires
        @ 90.3% WR — best stand-alone BTC head).
        """
        if not self._pool:
            return 0
        ...
        try:
            ...
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "pg_signal_repo.signal_evaluations_lgb_v9_3_btc_pure_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
    ...
    async def update_signal_evaluations_lgb_v9_2_pure(
        ...
        """Upsert ``probability_lgb_v9_2_pure`` on signal_evaluations row.
        ...
        migrations/add_probability_lgb_btc_pure_columns.sql (2026-05-24).
        Read by the v9_2_v12_combo_pure GHOST strategy. Idempotent via COALESCE.
        """
        if not self._pool:
            return 0
        ...
        try:
            ...
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "pg_signal_repo.signal_evaluations_lgb_v9_2_pure_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
    ...
    async def update_signal_evaluations_lgb_v12_pure(
        ...
        """Upsert ``probability_lgb_v12_pure`` on signal_evaluations row.
        ...
        try:
            ...
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "pg_signal_repo.signal_evaluations_lgb_v12_pure_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
    ...
    async def update_signal_evaluations_lgb_v9_5_xrp(
        ...
        """Upsert ``probability_lgb_v9_5_xrp`` on signal_evaluations row.

        Mirror of DBClient.update_signal_evaluations_lgb_v9_5_xrp for parity
        (lesson from PR #439 — keep the two writers verbatim). Column added
        ...
        try:
            ...
            n = int(result.split()[-1]) if result else 0
            log.debug(
                "pg_signal_repo.signal_evaluations_lgb_v9_5_xrp_upserted",
                window_ts=window_ts,
                asset=asset,
                timeframe=timeframe,
                eval_offset=eval_offset,
