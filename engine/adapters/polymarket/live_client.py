"""
Live Polymarket client -- real CLOB order signing and submission.

Extracted from ``engine/execution/polymarket_client.PolymarketClient``
(the ``paper_mode=False`` branches). Follows the margin_engine pattern:
``margin_engine/adapters/exchange/binance_margin.py`` (BinanceMarginAdapter).

All behavior is copied VERBATIM from the monolith -- zero logic changes.
The composition root selects this adapter when paper_mode is False.

Safety guards carried over from the original:
  - Requires ``LIVE_TRADING_ENABLED=true`` environment variable
  - Hard cap of $50 per single trade (LIVE_MAX_TRADE_USD)
...
class LivePolymarketClient(PolymarketClientPort):
    """
    Live Polymarket client -- real CLOB signing and order submission.
    """
    ...
    def __init__(
        ...
        self._log = logger.bind(component="polymarket_client", paper_mode=False)
    ...
    def _check_first_trade_warning(self, context: str, **log_kwargs) -> None:
        """Emit a warning on the very first live trade in this process."""
        if not self._live_first_trade_warned:
            self._live_first_trade_warned = True
            warnings.warn(
                f"First live Polymarket {context} trade being placed -- verify manually on "
                "polymarket.com/portfolio",
                RuntimeWarning,
                stacklevel=3,
            )
            self._log.warning("place_order.first_live_trade", **log_kwargs)
    ...
    async def place_fok_order(self, token_id: str, price: float, size: float) -> dict:
    ...
    async def place_rfq_order(self, token_id: str, direction: str, price: float, size: float, max_price: float = 0.65) -> tuple[str | None, float | None]:
    ...
    async def get_market_prices(self, market_slug: str) -> dict[str, Decimal]:
        """Fetch best-bid prices for YES and NO outcomes from CLOB."""
        client = self._ensure_client()
        try:
            def _fetch_prices():
    ...
    async def _fetch_all_positions(self) -> list:
        ...
        contains fewer than PAGE_SIZE records, at which point we've
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            while True:
                ...
                async with session.get(url, timeout=aiohttp.ClientTimeout(total=15)) as resp:
                    if resp.status != 200:
                        self._log.warning(
                            "live_client.positions_api_error",
                            status=resp.status,
                            offset=offset,
                        )
                        break
        ...
        if offset > 0:
            self._log.debug(
                "live_client.positions_paginated",
    ...
    async def list_recent_orders(self, since: Optional[datetime] = None, limit: int = 50) -> list[PolyOrderStatus]:
