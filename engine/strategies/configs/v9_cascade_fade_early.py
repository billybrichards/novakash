...
The actual specialisation lives in the YAML gate_params:
  ...
  * min_consecutive_pass_ticks=1 — cascade alpha decays fast (k=3 hurts).
  * VHC paths all disabled (VHC assumes high conviction is safer; we
    actively want low conviction).
...
_STRATEGY_ID = "v9_cascade_fade_early"
_VERSION = "1.0.0-ghost"


def evaluate_v9_cascade_fade_early(surface: "FullDataSurface") -> StrategyDecision:
    """Delegate to v9_ensemble gate stack with v9 LGB probability.

    Same swap-and-restore pattern as v10_lgb_only / v12_lgb_combo. We do
    NOT swap a different model in — we use v9's prod LGB (probability_lgb).
    The specialisation comes entirely from the gate_params (DOWN-only lock,
    CASCADE-only regime, k=1, low-conviction floor).
    """
    ...
    # Cold-start regime fallback (mirrors v10_lgb_only). If v4_regime is
    # None, the v4_regime gate would skip — give it a tradeable default.
    ...
    meta["v9_cascade_fade_specialist"] = True
