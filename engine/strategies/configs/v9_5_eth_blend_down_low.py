"""v9_5_eth_blend_down_low — DOWN-LOW conviction tier of the v9.5 ETH
BLEND ladder (GHOST).

Fires DOWN when probability_lgb_v9_5_eth falls inside the band
(down_threshold_min, down_threshold] — default (0.18, 0.24]. This is
the softest-conviction tier of the DOWN ladder; sibling strategies
v9_5_eth_blend_down_mid and v9_5_eth_blend_down_high cover the lower
(stronger-DOWN-conviction) bands.

See sibling v9_5_eth_blend_up_low.py for the full ladder layout and
design notes.

Band (0.18, 0.24]: n=214 OOF, mean WR ~91.2%, Wilson LB ~87%.

DOWN-only by design — UP-side overrides cannot flip this strategy into
firing UP. Asset: ETH only. Reads the EXISTING probability_lgb_v9_5_eth
column. GHOST mode by default. $5 max_position_usd.
"""
...
_STRATEGY_ID = "v9_5_eth_blend_down_low"
_VERSION = "1.0.0"

# Band (0.18, 0.24]: n=214 OOF, mean WR ~91.2%, Wilson LB ~87%.
_DEFAULT_DOWN_THRESHOLD = 0.24       # band upper bound (inclusive)
_DEFAULT_DOWN_THRESHOLD_MIN = 0.18   # band lower bound (exclusive)
...
def evaluate_v9_5_eth_blend_down_low(surface: "FullDataSurface") -> StrategyDecision:
    ...
    # DOWN-only band check: (down_threshold_min, down_threshold] — lower
    # exclusive, upper inclusive (the lower edge routes to down_mid).
    # UP direction is intentionally not implemented; this is a ladder-tier
    # strategy, not a two-sided strategy.
    ...
    return StrategyDecision(
        ...
        strategy_id=_STRATEGY_ID,
        strategy_version=_VERSION,
        entry_reason="v9_5_eth_blend_down_low_pass",
        skip_reason=None,
        metadata=meta,
    )
