"""v9_5_eth_blend_down_high — DOWN-HIGH floor tier of the v9.5 ETH BLEND
ladder (GHOST).

Fires DOWN when probability_lgb_v9_5_eth falls inside the band
(down_threshold_min, down_threshold] — default (-1.0, 0.12]. The lower
bound is an impossible value (calibrated probability is in [0, 1]), so
this strategy effectively fires on any p <= 0.12 — the strongest DOWN
conviction region of the v9.5 ETH BLEND output.

See sibling v9_5_eth_blend_up_low.py for the full ladder layout.

Band [0, 0.12]: n=53 OOF, mean WR ~96%, Wilson LB ~88%.
...
_STRATEGY_ID = "v9_5_eth_blend_down_high"
_VERSION = "1.0.0"

# Floor tier: upper bound 0.12, lower bound impossible (-1.0 — calibrated
# p never goes below 0.0). Effectively a one-sided p <= 0.12 check.
_DEFAULT_DOWN_THRESHOLD = 0.12
_DEFAULT_DOWN_THRESHOLD_MIN = -1.0
...
def evaluate_v9_5_eth_blend_down_high(surface: "FullDataSurface") -> StrategyDecision:
    ...
    # DOWN-only band check (floor tier — lower bound -1.0 is impossible).
    ...
    return StrategyDecision(
        ...
        strategy_id=_STRATEGY_ID,
        strategy_version=_VERSION,
        entry_reason="v9_5_eth_blend_down_high_pass",
        skip_reason=None,
        metadata=meta,
    )
