"""v9_3_btc_down_solo — DOWN-only BTC 5m strategy on probability_lgb_v9_3_btc (GHOST).

Fires ONLY on the DOWN side reading probability_lgb_v9_3_btc — the BLEND column
for the v9.3 BTC-trained model (data_surface.py line 314). This is the same
column read by v9_3_btc_blend and v9_3_btc_tight_blend, but at a different
operating point that fills a coverage gap those strategies miss:
  - v9_3_btc_blend:       DOWN p <= 0.20
  - v9_3_btc_tight_blend: DOWN p <= 0.065
  - THIS STRATEGY:        DOWN p <= 0.50  (the 0.20 < p <= 0.50 pocket)
...
WHY DOWN-ONLY?
  Today's live signal evaluation sweep (RDS note #694, 2026-05-25) shows:
    DOWN p <= 0.50: 83.3% WR n=18  (today)
    DOWN p <= 0.48: 80.0% WR n=15  (tighter alternative)
  The UP side at p >= 0.65 sits at ~55% WR — too close to random, excluded.
  If UP recovers with more data or at higher thresholds, Billy can create a
  companion v9_3_btc_up_solo.

Operating point (RDS note #694, today's live sweep, 2026-05-25):
  - DOWN when probability_lgb_v9_3_btc <= 0.50
           -> 83.3% WR n=18 (today)
  - NO UP side (v9_3_btc UP is ~55% WR — excluded)
  - eval_offset in [60, 180]  — best band consistent with v9_2_eth_solo sweep
  - min_consecutive_pass_ticks=1

COVERAGE GAP THIS FILLS:
  v9_3_btc_blend fires DOWN only when p <= 0.20. This strategy fires DOWN
  when p is in the range (0.20, 0.50] — a zone that existing strategies ignore
  but today's data shows is 83.3% WR. Fires do NOT overlap v9_3_btc_blend
  at the same window (different threshold gates); no market conflict.
...
References:
  ...
  data_surface.py line 314 — probability_lgb_v9_3_btc definition
  Sibling strategies: v9_3_btc_blend (DOWN p<=0.20), v9_3_btc_tight_blend
...
# NO up threshold — UP side is ~55% WR (excluded).
_DEFAULT_DOWN_THRESHOLD = 0.50
...
# collide with sibling strategies (v9_3_btc_blend, v9_3_btc_tight_blend,
# v9_3_btc_pure_lgb).
...
def evaluate_v9_3_btc_down_solo(surface: "FullDataSurface") -> StrategyDecision:
    ...
    # DOWN-only strategy. UP side (~55% WR, RDS #694) is too weak to trade.
    if p_btc <= down_threshold:
        direction = "DOWN"
    else:
