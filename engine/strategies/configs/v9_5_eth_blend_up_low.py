"""v9_5_eth_blend_up_low — UP-LOW conviction tier of the v9.5 ETH BLEND
ladder (GHOST).

Fires UP when probability_lgb_v9_5_eth falls inside the band
[up_threshold, up_threshold_max) — default [0.78, 0.82). This is the
softest-conviction tier of the UP ladder; sibling strategies
v9_5_eth_blend_up_mid and v9_5_eth_blend_up_high cover the higher tiers
(see v9_5_eth_blend_up_low.yaml header for the full ladder layout and
band provenance).

UP-only by design — DOWN-side overrides cannot flip this strategy into
firing DOWN. This is intentional ladder hygiene: each tier handles one
conviction slice of one direction; DOWN tiers are separate strategies.

Asset: ETH only. The v9.5 ETH model is calibrated for ETH only; the
defensive asset guard SKIPs any non-ETH surface even if the registry
ever wires one in.

Module-local _consec_state: every ladder strategy gets its own
consecutive-tick state dict so sibling-tier and sibling-direction
counters cannot leak. Tested explicitly (see TestCrossStrategyIndependence
in the matching test file).

Reads the EXISTING probability_lgb_v9_5_eth column — no new emission
needed, no overlap with the parallel pure-LGB column from
`feat/v9_5_eth_pure_lgb_strategy`.

GHOST mode by default per feedback_no_auto_promote.md. $5
max_position_usd for the initial shadow window.
"""
...
_STRATEGY_ID = "v9_5_eth_blend_up_low"
_VERSION = "1.0.0"

# Band [0.78, 0.82): n=141 OOF, mean WR ~88.7%, Wilson LB ~83%.
_DEFAULT_UP_THRESHOLD = 0.78        # band lower bound (inclusive)
_DEFAULT_UP_THRESHOLD_MAX = 0.82    # band upper bound (exclusive)

# Broad eval-offset band — selectivity comes from the conviction band,
# NOT the eval-offset gate.
...
# Module-local consecutive-tick state. Maps
# (window_ts, direction) -> (count, last_seen_ts). Kept module-local so
# this tier's counters cannot collide with sibling ladder tiers or the
# DOWN-side strategies.
...
def evaluate_v9_5_eth_blend_up_low(surface: "FullDataSurface") -> StrategyDecision:
    ...
    # Defensive asset guard. Belt-and-braces — the v9.5 ETH model is not
    # calibrated for other assets.
    ...
    # UP-only band check. [up_threshold, up_threshold_max) — lower
    # inclusive, upper exclusive (the upper edge routes to up_mid).
    # DOWN direction is intentionally not implemented; this is a
    # ladder-tier strategy, not a two-sided strategy.
    ...
    return StrategyDecision(
        ...
        strategy_id=_STRATEGY_ID,
        strategy_version=_VERSION,
        entry_reason="v9_5_eth_blend_up_low_pass",
        skip_reason=None,
        metadata=meta,
    )
