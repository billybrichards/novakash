"""tickformer_v17_sniper_eth — V4 ETH/XRP TickFormer SHADOW (GHOST soak) strategy.

Thin wrapper that points the shared TickFormer evaluation core at
``probability_tickformer_v17_eth`` (NUMERIC) emitted by the
...
_STRATEGY_ID = "tickformer_v17_sniper_eth"
_VERSION = "1.0.0"


def evaluate_tickformer_v17_sniper_eth(
    ...
    return evaluate_tickformer_strategy(
        surface,
        prob_column="probability_tickformer_v17_eth",
        strategy_id=_STRATEGY_ID,
        version=_VERSION,
        default_up_threshold=0.88,
        default_rem_min=60,
        default_rem_max=140,
        entry_reason_label="tickformer_v17_sniper_eth_pass",
        not_available_reason="tickformer_v17_eth_not_available",
    )
