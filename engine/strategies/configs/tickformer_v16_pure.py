...
Reads the NEW probability_tickformer_v16 (NUMERIC) and
tickformer_trade_signal (UP/DOWN/HOLD) fields emitted by timesfm-service
(sister PR on the magic-model repo). The TickFormer v16 model is a
hybrid transformer that combines per-tick attention with a multi-step
autoregressive inference loop — internally referred to as the "magic
model" — to project residual window-outcome direction.

Conviction tiers from the v16 spec (offline backtest pockets):
    thr 0.85, eval_offset_remaining 60s   -> ~95% WR
    thr 0.90, eval_offset_remaining 60s   -> ~96% WR
    thr 0.85, eval_offset_remaining 180s  -> ~94% WR
    thr 0.85, eval_offset_remaining 220s  -> ~88% WR (dream tier)

This strategy is registered SHADOW by default — Billy promotes manually
per feedback_no_auto_promote.md (post-2026-04-17 rule). A
`shadow_only: bool` gate_param flag forces SKIP-with-decision-record
behaviour even if the YAML mode is mis-set; it must be flipped to false
in YAML before any real fire is emitted.

DOWN side is symmetric: a low probability_tickformer_v16 (1 - p_up) is
the v16 DOWN edge. The default DOWN threshold is the complement of the
UP threshold (p <= 0.15 == 1 - 0.85). tickformer_trade_signal must
agree with the chosen direction when present (UP/DOWN); HOLD or None
does not block — only an explicit *opposite* signal blocks the fire.

Sized by Kelly (collateral_pct) capped at max_position_usd via YAML.
$5 max_position_usd for the shadow window; raise once shadow data
validates the magic-model tiers under live noise.

Sibling timesfm PR: magic-model branch (probability_tickformer_v16 +
tickformer_trade_signal emission on /v4/snapshot.timescales.5m + new
GET /v5/probability and GET /v5/trade_signal endpoints).
Engine precedent for SHADOW pattern: v9_5_xrp_up_solo.py.
"""
...
# Default operating point — Tier B from the v16 spec (thr 0.85 / 180s).
# Billy picks the live tier in YAML by overriding up_threshold +
# eval_offset_remaining_min. Defaults stay conservative.
_DEFAULT_UP_THRESHOLD = 0.85
_DEFAULT_DOWN_THRESHOLD = 0.15  # 1 - up_threshold, symmetric

# eval_offset_remaining (NOT eval_offset): seconds left in the 5m window
# when the tick was scored. The v16 magic model degrades sharply as the
# window approaches expiry — late ticks lose attention context. The
# 60s/180s/220s tiers in the spec are remaining-time pockets.
_DEFAULT_EVAL_OFFSET_REMAINING_MIN = 60
_DEFAULT_EVAL_OFFSET_REMAINING_MAX = 240  # admit Tier-D dream zone by default

# Standard cap/sizing defaults (mirror v9_5_xrp_up_solo for like-for-like
# shadow comparisons against the v9.5 PURE strategies).
...
# Flipping this off in YAML is the explicit "go live" switch.
_DEFAULT_SHADOW_ONLY = True

# Consecutive-tick state. Module-local so the magic-model strategy's
# state cannot collide with v9_2_eth_solo / v9_5_xrp_up_solo etc.
...
def _eval_offset_remaining(surface: "FullDataSurface") -> int | None:
    ...
    Prefers an explicit ``eval_offset_remaining`` field if the surface
    exposes one (forward-compat with a future surface field); otherwise
    computes 300 - eval_offset on the assumption of a 300s window.
    """
...
def evaluate_tickformer_v16_pure(surface: "FullDataSurface") -> StrategyDecision:
    p = getattr(surface, "probability_tickformer_v16", None)
    ...
    if p is None:
        return _skip(
            "tickformer_v16_not_available",
            {"probability_tickformer_v16": None, "asset": asset},
        )
    p = float(p)
    ...
    return StrategyDecision(
        ...
        strategy_id=_STRATEGY_ID,
        strategy_version=_VERSION,
        entry_reason="tickformer_v16_pure_pass",
        skip_reason=None,
        metadata=meta,
    )
