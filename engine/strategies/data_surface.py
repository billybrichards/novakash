...
class FullDataSurface:
    ...
    # v9.2-style XRP 5m LGB head — raw probability for the XRP-trained model.
    # Emitted by timesfm-service on /v4/snapshot.timescales.5m when
    # V9_2_XRP_ENABLED=true (companion bg-agent-1 timesfm PR, 2026-05-20).
    # Read ONLY by the v9_2_xrp_raw_lgb GHOST strategy (asset=XRP, 5m).
    # Recommended operating point per hub note #550: UP >= 0.92, DOWN <= 0.06,
    # eval_offset in [60, 210]. Projected WR ~92-94% on the v1 test set.
    ...
    probability_lgb_v9_2_xrp: Optional[float] = None
    ...
    def get_surface(
        ...
        return FullDataSurface(
            ...
            probability_lgb_v9_2_xrp=(
                float(ts_data["probability_lgb_v9_2_xrp"])
                if ts_data.get("probability_lgb_v9_2_xrp") is not None
                else None
            ),
