"""
EngineRuntime — owns runtime behaviour (tasks, loops, callbacks).

Extracted from Orchestrator during Phase 5 refactor. All methods (except
__init__) were copied verbatim from strategies/orchestrator.py. __init__
unpacks components from a CompositionRoot and patches feed/component
callbacks that CompositionRoot left as None.
"""
...
from data.aggregator import MarketAggregator  # noqa: F401
from data.feeds.binance_ws import BinanceWebSocketFeed  # noqa: F401
from data.feeds.chainlink_rpc import ChainlinkRPCFeed  # noqa: F401
...
from data.feeds.coinglass_api import CoinGlassAPIFeed  # noqa: F401
from data.feeds.coinglass_enhanced import CoinGlassEnhancedFeed
from evaluation.claude_evaluator import ClaudeEvaluator  # noqa: F401
from evaluation.post_resolution_evaluator import PostResolutionEvaluator  # noqa: F401
from data.feeds.polymarket_ws import PolymarketWebSocketFeed  # noqa: F401
from data.feeds.polymarket_5min import Polymarket5MinFeed  # noqa: F401
from polymarket_browser.service import PlaywrightService  # noqa: F401
...
from execution.opinion_client import OpinionClient  # noqa: F401
from execution.order_manager import OrderManager  # noqa: F401
from execution.polymarket_client import PolymarketClient  # noqa: F401
from execution.risk_manager import RiskManager  # noqa: F401
from persistence.db_client import DBClient, DBClientLegacyShim  # noqa: F401
from persistence.tick_recorder import TickRecorder
from signals.arb_scanner import ArbScanner  # noqa: F401
from signals.cascade_detector import CascadeDetector  # noqa: F401
from signals.regime_classifier import RegimeClassifier  # noqa: F401
from signals.vpin import VPINCalculator  # noqa: F401
from strategies.five_min_vpin import FiveMinVPINStrategy  # noqa: F401
from signals.twap_delta import TWAPTracker  # noqa: F401
...
class EngineRuntime:
    ...
    Takes a CompositionRoot, unpacks its components onto self, patches the
    feed/component callbacks CompositionRoot left as None, and manages the
    engine lifecycle (start / run / stop, all loops).
    """

    def __init__(self, root: CompositionRoot) -> None:
        self._root = root

        # ── Unpack composed components onto self for backward-compat ─────────
        # (every method body below references self._X; these stay the same)
        self._settings = root._settings
        self._db = root._db
        self._aggregator = root._aggregator
        self._alerter = root._alerter
        self._vpin_calc = root._vpin_calc
        self._cascade = root._cascade
        self._arb_scanner = root._arb_scanner
        self._regime = root._regime
        self._poly_client = root._poly_client
        self._opinion_client = root._opinion_client
        self._order_manager = root._order_manager
        self._risk_manager = root._risk_manager
        self._redeemer = root._redeemer
        self._playwright = root._playwright
        self._cg_enhanced = root._cg_enhanced
        self._cg_feeds = root._cg_feeds
        self._claude_evaluator = root._claude_evaluator
        self._post_resolution_evaluator = root._post_resolution_evaluator
        self._five_min_strategy = root._five_min_strategy
        self._five_min_feed = root._five_min_feed
        self._fifteen_min_feed = root._fifteen_min_feed
        ...
        self._geoblock_active: bool = False
        self._execute_uc = None
        self._manual_trade_notify_event: asyncio.Event = asyncio.Event()
        self._resolved_by_order_manager: set = set()
        self._last_sp_result = None
        self._reconciler = root._reconciler
        ...
        if self._vpin_calc is not None:
            self._vpin_calc._on_signal = self._on_vpin_signal
        if self._cascade is not None:
            self._cascade._on_signal = self._on_cascade_signal
        if self._arb_scanner is not None:
            self._arb_scanner._on_opportunities = self._on_arb_opportunities
        if self._order_manager is not None:
            self._order_manager._on_resolution = self._on_order_resolution
        if self._five_min_feed is not None:
            self._five_min_feed._on_window_signal = self._on_five_min_window
        if self._fifteen_min_feed is not None:
            self._fifteen_min_feed._on_window_signal = self._on_fifteen_min_window
        if self._binance_feed is not None:
            self._binance_feed._on_trade = self._on_binance_trade
        if self._binance_spot_feed is not None:
            self._binance_spot_feed._on_trade = self._on_binance_spot_trade
        if self._coinglass_feed is not None:
            self._coinglass_feed._on_oi = self._on_oi_update
        if self._polymarket_feed is not None:
            self._polymarket_feed._on_book = self._on_polymarket_book
        # Repatch five_min_strategy geoblock closure to hit live runtime state
        if self._five_min_strategy is not None:
            self._five_min_strategy._geoblock_check_fn = lambda: self._geoblock_active
