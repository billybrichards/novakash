"""Unit tests for v9_5_eth_pure_down_solo GHOST strategy.
...
- DOWN threshold gating: fires at p=0.25, fires below, just above skips
...
- Cross-strategy independence: module-local _consec_state doesn't leak
  into sibling strategies (v9_5_eth_pure_lgb, v9_5_eth_blend)
...
from strategies.configs.v9_5_eth_pure_down_solo import evaluate_v9_5_eth_pure_down_solo
...
def _make_surface(**overrides) -> FullDataSurface:
    """Default surface for v9_5_eth_pure_down_solo tests.

    asset=ETH, eval_offset=120 (in band [60, 180]),
    probability_lgb_v9_5_eth_pure=0.20 (below the 0.25 DOWN threshold).
    ...
    defaults = dict(
        ...
        probability_lgb_v9_5_eth_pure=0.20,
    )
...
def _clear_consec_state():
    from strategies.configs import v9_5_eth_pure_down_solo
    v9_5_eth_pure_down_solo._consec_state.clear()
    yield
    v9_5_eth_pure_down_solo._consec_state.clear()
...
class TestNullProbabilityHandling:
    def test_model_not_loaded_skips_cleanly(self):
        ...
        with _params():
            d = evaluate_v9_5_eth_pure_down_solo(surface)
        assert d.action == "SKIP"
        assert d.skip_reason == "v9_5_eth_pure_model_not_loaded"
        assert d.strategy_id == "v9_5_eth_pure_down_solo"
...
class TestAssetGuard:
    def test_btc_surface_skips_even_with_low_prob(self):
        ...
        surface = _make_surface(asset="BTC", probability_lgb_v9_5_eth_pure=0.10)
        with _params():
            d = evaluate_v9_5_eth_pure_down_solo(surface)
        assert d.action == "SKIP"
    ...
    def test_xrp_surface_skips(self):
        surface = _make_surface(asset="XRP", probability_lgb_v9_5_eth_pure=0.10)
        with _params():
            d = evaluate_v9_5_eth_pure_down_solo(surface)
        assert d.action == "SKIP"
        assert d.skip_reason == "wrong_asset"
...
class TestEvalOffsetBand:
    ...
    def test_at_min_60_fires(self):
        surface = _make_surface(eval_offset=60, probability_lgb_v9_5_eth_pure=0.20)
        with _params():
            d = evaluate_v9_5_eth_pure_down_solo(surface)
    ...
    def test_at_max_180_fires(self):
        surface = _make_surface(eval_offset=180, probability_lgb_v9_5_eth_pure=0.20)
        with _params():
            d = evaluate_v9_5_eth_pure_down_solo(surface)
        assert d.action == "TRADE"
        assert d.direction == "DOWN"
    ...
    def test_offset_outside_band_skips_even_with_low_prob(self):
        ...
        surface = _make_surface(eval_offset=240, probability_lgb_v9_5_eth_pure=0.05)
        with _params():
            d = evaluate_v9_5_eth_pure_down_solo(surface)
        assert d.action == "SKIP"
        assert d.skip_reason == "outside_eval_band"
...
class TestDownThreshold:
    def test_down_fires_at_threshold_0_25(self):
        """Exactly at DOWN threshold 0.25 — fires DOWN."""
        surface = _make_surface(probability_lgb_v9_5_eth_pure=0.25)
        with _params():
            d = evaluate_v9_5_eth_pure_down_solo(surface)
        assert d.action == "TRADE"
        assert d.direction == "DOWN"
        assert d.strategy_id == "v9_5_eth_pure_down_solo"
        assert d.entry_reason == "v9_5_eth_pure_down_solo_pass"

    def test_down_fires_at_0_24(self):
        """p=0.24 < 0.25 — fires DOWN."""
        surface = _make_surface(probability_lgb_v9_5_eth_pure=0.24)
        with _params():
            d = evaluate_v9_5_eth_pure_down_solo(surface)
        assert d.action == "TRADE"
        assert d.direction == "DOWN"
    ...
    def test_down_fires_well_below_threshold(self):
        surface = _make_surface(probability_lgb_v9_5_eth_pure=0.05)
        with _params():
            d = evaluate_v9_5_eth_pure_down_solo(surface)
        assert d.action == "TRADE"
        assert d.direction == "DOWN"
    ...
    def test_just_above_down_threshold_skips(self):
        """0.251 > 0.25 — above DOWN threshold."""
        surface = _make_surface(probability_lgb_v9_5_eth_pure=0.251)
        with _params():
            d = evaluate_v9_5_eth_pure_down_solo(surface)
        assert d.action == "SKIP"
...
class TestNoUpFires:
    def test_high_prob_does_not_fire_up(self):
        """p=0.90 is above 0.25 — no UP direction, skips."""
        surface = _make_surface(probability_lgb_v9_5_eth_pure=0.90)
        with _params():
            d = evaluate_v9_5_eth_pure_down_solo(surface)
        assert d.action == "SKIP"
        assert d.skip_reason == "conviction_below_threshold"
    ...
    def test_p_0_82_does_not_fire_up(self):
        """p=0.82 — strong signal in other strats, but DOWN-only skips this."""
        surface = _make_surface(probability_lgb_v9_5_eth_pure=0.82)
        with _params():
            d = evaluate_v9_5_eth_pure_down_solo(surface)
        assert d.action == "SKIP"
        assert d.skip_reason == "conviction_below_threshold"
    ...
    def test_p_0_99_skips(self):
        ...
        surface = _make_surface(probability_lgb_v9_5_eth_pure=0.99)
        with _params():
            d = evaluate_v9_5_eth_pure_down_solo(surface)
        assert d.action == "SKIP"
        assert d.skip_reason == "conviction_below_threshold"
...
class TestFillBandGate:
    def test_down_fires_with_fill_0_80_below_cap(self):
        """DOWN at p=0.20 + fill=0.80 (< 0.90 cap) → TRADE."""
    ...
    def test_down_skips_with_fill_0_91_above_cap(self):
        """DOWN at p=0.20 + fill=0.91 (>= 0.90 cap) → SKIP fill_above_down_cap."""
        surface = _make_surface(
            probability_lgb_v9_5_eth_pure=0.20,
            eval_offset=120,
...
class TestRuntimeOverride:
    def test_runtime_override_tightens_down_threshold(self):
        ...
        with _params():
            d = evaluate_v9_5_eth_pure_down_solo(surface)
        assert d.action == "TRADE"
        # Tighten to 0.20 — p=0.24 is now > 0.20, SKIP.
        with _params(down_threshold=0.20):
            d = evaluate_v9_5_eth_pure_down_solo(surface)
        assert d.action == "SKIP"
        assert d.skip_reason == "conviction_below_threshold"

    def test_runtime_override_narrows_eval_band(self):
        surface = _make_surface(eval_offset=170, probability_lgb_v9_5_eth_pure=0.20)
...
class TestMetadataShape:
    def test_metadata_contains_probability_on_down_trade(self):
        surface = _make_surface(probability_lgb_v9_5_eth_pure=0.20, eval_offset=120)
        with _params():
            d = evaluate_v9_5_eth_pure_down_solo(surface)
        assert d.action == "TRADE"
        assert d.metadata["probability_lgb_v9_5_eth_pure"] == pytest.approx(0.20)
        assert d.metadata["down_threshold"] == pytest.approx(0.25)
        ...
        assert d.metadata["strategy_id"] == "v9_5_eth_pure_down_solo"
        assert d.metadata["strategy_version"] == "1.0.0"

    def test_metadata_contains_sizing_on_trade(self):
        surface = _make_surface(probability_lgb_v9_5_eth_pure=0.20)
        with _params():
            d = evaluate_v9_5_eth_pure_down_solo(surface)
        assert d.action == "TRADE"
...
class TestConsecutiveTicks:
    ...
    def test_runtime_override_requires_two_ticks(self):
        ...
        with _params(min_consecutive_pass_ticks=2):
            d2 = evaluate_v9_5_eth_pure_down_solo(surface)
...
class TestCrossStrategyIndependence:
    ...
    def test_consec_state_isolated_from_pure_lgb(self):
        """Mutating v9_5_eth_pure_down_solo._consec_state must NOT affect
        v9_5_eth_pure_lgb._consec_state (separate module-locals).
        """
        from strategies.configs import v9_5_eth_pure_down_solo as down_solo
        from strategies.configs import v9_5_eth_pure_lgb as pure_lgb
        ...
        # pure_lgb's state remains untouched.
        assert len(pure_lgb._consec_state) == 0

    def test_consec_state_isolated_from_eth_blend(self):
        """Module-local state is isolated from v9_5_eth_blend."""
        from strategies.configs import v9_5_eth_pure_down_solo as down_solo
        from strategies.configs import v9_5_eth_blend as eth_blend

        down_solo._consec_state.clear()
        eth_blend._consec_state.clear()
        assert down_solo._consec_state is not eth_blend._consec_state

        surface = _make_surface(probability_lgb_v9_5_eth_pure=0.20)
        with _params():
            evaluate_v9_5_eth_pure_down_solo(surface)
        assert len(eth_blend._consec_state) == 0
