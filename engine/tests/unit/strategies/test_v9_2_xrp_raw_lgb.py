"""Unit tests for v9_2_xrp_raw_lgb GHOST strategy.

Coverage:
- model-not-loaded SKIP (v9_2_xrp_model_not_loaded) — forward-compat path
...
- TRADE path: UP at 0.92, DOWN at 0.06 per hub note #550 recommendation
- gate_params runtime override respected
- Metadata shape on TRADE includes probability_lgb_v9_2_xrp + thresholds
- Strategy is asset-restricted: same threshold passes for BTC/ETH surface SKIPs
...
from strategies.configs.v9_2_xrp_raw_lgb import evaluate_v9_2_xrp_raw_lgb
...
def _make_surface(**overrides) -> FullDataSurface:
    ...
    asset=XRP, eval_offset=150 (in band [60, 210]), hour_utc=12,
    probability_lgb_v9_2_xrp=0.95 (above the 0.92 UP threshold).
    """
    defaults = dict(
        asset="XRP", timescale="5m",
        window_ts=1713009600,
        eval_offset=150, assembled_at=time.time(),
        current_price=2.500, open_price=2.490,
...
_BASE_PARAMS: dict[str, Any] = {
    # XRP v1.0.0 thresholds per hub note #550 corrected WR-at-threshold table.
    # XRP did NOT show δ=180 weakness like ETH did, so eval_offset band is
    # wider [60, 210] than ETH's v1.1.0 [120, 210].
    "up_threshold": 0.92,
    "down_threshold": 0.06,
    "eval_offset_min": 60,
    "eval_offset_max": 210,
...
def _clear_consec_state():
    ...
    from strategies.configs import v9_2_xrp_raw_lgb
    v9_2_xrp_raw_lgb._consec_state.clear()
    yield
    v9_2_xrp_raw_lgb._consec_state.clear()
...
class TestForwardCompat:
    def test_model_not_loaded_skips_cleanly(self):
        """When the snapshot has no probability_lgb_v9_2_xrp, the strategy
        SKIPs with a clear reason — no crash."""
        surface = _make_surface(probability_lgb_v9_2_xrp=None)
        with _params():
            d = evaluate_v9_2_xrp_raw_lgb(surface)
        assert d.action == "SKIP"
        assert d.skip_reason == "v9_2_xrp_model_not_loaded"
        # Decision still includes the strategy id even on SKIP.
        assert d.strategy_id == "v9_2_xrp_raw_lgb"
...
class TestAssetGuard:
    def test_btc_surface_skips_even_with_high_prob(self):
        """XRP strategy refuses to fire on a BTC surface even at high
        conviction — defensive guard since the model is XRP-only."""
        surface = _make_surface(asset="BTC", probability_lgb_v9_2_xrp=0.95)
        with _params():
            d = evaluate_v9_2_xrp_raw_lgb(surface)
        assert d.action == "SKIP"
    ...
    def test_eth_surface_skips(self):
        surface = _make_surface(asset="ETH", probability_lgb_v9_2_xrp=0.95)
        with _params():
            d = evaluate_v9_2_xrp_raw_lgb(surface)
        assert d.action == "SKIP"
        assert d.skip_reason == "wrong_asset"
...
class TestEvalOffsetBand:
    def test_below_min_skips(self):
        """eval_offset=55 is below the XRP band min of 60."""
        surface = _make_surface(eval_offset=55)
        with _params():
            d = evaluate_v9_2_xrp_raw_lgb(surface)
        assert d.action == "SKIP"
    ...
    def test_above_max_skips(self):
        """eval_offset=220 is above the XRP band max of 210."""
    ...
    def test_at_min_60_fires(self):
        surface = _make_surface(eval_offset=60, probability_lgb_v9_2_xrp=0.95)
        with _params():
            d = evaluate_v9_2_xrp_raw_lgb(surface)
    ...
    def test_at_max_210_fires(self):
        surface = _make_surface(eval_offset=210, probability_lgb_v9_2_xrp=0.95)
        with _params():
            d = evaluate_v9_2_xrp_raw_lgb(surface)
        assert d.action == "TRADE"
        assert d.direction == "UP"
...
class TestThresholds:
    def test_up_fires_at_0_92(self):
        """XRP UP threshold is 0.92 per hub note #550."""
        surface = _make_surface(probability_lgb_v9_2_xrp=0.92)
        with _params():
            d = evaluate_v9_2_xrp_raw_lgb(surface)
        assert d.action == "TRADE"
        assert d.direction == "UP"
        assert d.strategy_id == "v9_2_xrp_raw_lgb"
        assert d.entry_reason == "v9_2_xrp_raw_lgb_pass"

    def test_up_fires_above_0_92(self):
        surface = _make_surface(probability_lgb_v9_2_xrp=0.99)
        with _params():
            d = evaluate_v9_2_xrp_raw_lgb(surface)
        assert d.action == "TRADE"
        assert d.direction == "UP"

    def test_down_fires_at_0_06(self):
        """XRP DOWN threshold is 0.06 per hub note #550."""
        surface = _make_surface(probability_lgb_v9_2_xrp=0.06)
        with _params():
            d = evaluate_v9_2_xrp_raw_lgb(surface)
        assert d.action == "TRADE"
        assert d.direction == "DOWN"

    def test_down_fires_below_0_06(self):
        surface = _make_surface(probability_lgb_v9_2_xrp=0.02)
        with _params():
            d = evaluate_v9_2_xrp_raw_lgb(surface)
        assert d.action == "TRADE"
        assert d.direction == "DOWN"
    ...
    def test_between_thresholds_skips(self):
        # 0.50 is neither >= 0.92 nor <= 0.06
        surface = _make_surface(probability_lgb_v9_2_xrp=0.50)
        with _params():
            d = evaluate_v9_2_xrp_raw_lgb(surface)
        assert d.action == "SKIP"
    ...
    def test_just_below_up_skips(self):
        # 0.91 < 0.92 — below the UP threshold
        surface = _make_surface(probability_lgb_v9_2_xrp=0.91)
        with _params():
            d = evaluate_v9_2_xrp_raw_lgb(surface)
        assert d.action == "SKIP"
        assert d.skip_reason == "conviction_below_threshold"
    ...
    def test_just_above_down_skips(self):
        # 0.07 > 0.06 — above the DOWN threshold
        surface = _make_surface(probability_lgb_v9_2_xrp=0.07)
        with _params():
            d = evaluate_v9_2_xrp_raw_lgb(surface)
        assert d.action == "SKIP"
        assert d.skip_reason == "conviction_below_threshold"
...
class TestRuntimeOverride:
    def test_runtime_override_lowers_up_threshold(self):
        ...
        surface = _make_surface(probability_lgb_v9_2_xrp=0.88)
        # with default 0.92 threshold: SKIP
        with _params():
            d = evaluate_v9_2_xrp_raw_lgb(surface)
        assert d.action == "SKIP"
    ...
    def test_runtime_override_narrows_eval_band(self):
        # surface at eval_offset=90 fires under default [60, 210]
        surface = _make_surface(eval_offset=90, probability_lgb_v9_2_xrp=0.95)
        with _params():
            d = evaluate_v9_2_xrp_raw_lgb(surface)
        assert d.action == "TRADE"
        # narrow to [120, 210] — 90 now out of band
...
class TestMetadata:
    def test_metadata_contains_probability_on_trade(self):
        surface = _make_surface(probability_lgb_v9_2_xrp=0.95)
        with _params():
            d = evaluate_v9_2_xrp_raw_lgb(surface)
        assert d.action == "TRADE"
        assert "probability_lgb_v9_2_xrp" in d.metadata
        assert d.metadata["probability_lgb_v9_2_xrp"] == pytest.approx(0.95)
    ...
    def test_confidence_score_is_high_at_0_95(self):
        # |0.95 - 0.5| * 2 = 0.90 >= 0.40 -> HIGH
        surface = _make_surface(probability_lgb_v9_2_xrp=0.95)
        with _params():
            d = evaluate_v9_2_xrp_raw_lgb(surface)
        assert d.action == "TRADE"
