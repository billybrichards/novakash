...
- Direction-aware fill-band gate:
    ...
    UP + fill=0.55 (< 0.60 floor)  → SKIP fill_below_up_floor
...
from strategies.configs.v9_5_xrp_up_solo import evaluate_v9_5_xrp_up_solo
...
def _clear_consec_state():
    from strategies.configs import v9_5_xrp_up_solo
    v9_5_xrp_up_solo._consec_state.clear()
    yield
    v9_5_xrp_up_solo._consec_state.clear()
...
class TestMetadataShape:
    def test_metadata_contains_probability_on_up_trade(self):
        ...
        assert d.metadata["strategy_id"] == "v9_5_xrp_up_solo"
...
class TestConsecutiveTicks:
    ...
    def test_runtime_override_requires_two_ticks(self):
        ...
        with _params(min_consecutive_pass_ticks=2):
            d2 = evaluate_v9_5_xrp_up_solo(surface)
...
class TestCrossStrategyIndependence:
    ...
    def test_consec_state_isolated_from_xrp_blend(self):
        """Mutating v9_5_xrp_up_solo._consec_state must NOT affect
        v9_5_xrp_blend._consec_state (separate module-locals).
        """
        ...
        # xrp_blend's state remains untouched.
        assert len(xrp_blend._consec_state) == 0

    def test_consec_state_isolated_from_late_band(self):
        """Module-local state is isolated from v9_5_xrp_late_band_AB_blend."""
        from strategies.configs import v9_5_xrp_up_solo as up_solo
