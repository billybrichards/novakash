"""Unit tests for v9_5_eth_blend_down_mid GHOST strategy (DOWN-MID tier).

Band gating: (0.12, 0.18] — lower bound EXCLUSIVE, upper bound INCLUSIVE.
...
from strategies.configs.v9_5_eth_blend_down_mid import (
    evaluate_v9_5_eth_blend_down_mid,
)
...
def _make_surface(**overrides) -> FullDataSurface:
    """Default ETH surface — probability 0.15 (inside DOWN-MID band)."""
    defaults = dict(
...
class TestNullProbabilityHandling:
    def test_model_not_loaded_skips_cleanly(self):
        ...
        assert d.strategy_id == "v9_5_eth_blend_down_mid"


class TestAssetGuard:
    def test_btc_surface_skips(self):
        surface = _make_surface(asset="BTC", probability_lgb_v9_5_eth=0.15)
        with _params():
            d = evaluate_v9_5_eth_blend_down_mid(surface)
        assert d.action == "SKIP"
    ...
    def test_xrp_surface_skips(self):
        surface = _make_surface(asset="XRP", probability_lgb_v9_5_eth=0.15)
        with _params():
            d = evaluate_v9_5_eth_blend_down_mid(surface)
        assert d.action == "SKIP"
        assert d.skip_reason == "wrong_asset"
...
class TestEvalOffsetBand:
    ...
    def test_at_min_60_fires(self):
        surface = _make_surface(eval_offset=60, probability_lgb_v9_5_eth=0.15)
        with _params():
            d = evaluate_v9_5_eth_blend_down_mid(surface)
    ...
    def test_at_max_240_fires(self):
        surface = _make_surface(eval_offset=240, probability_lgb_v9_5_eth=0.15)
        with _params():
            d = evaluate_v9_5_eth_blend_down_mid(surface)
        assert d.action == "TRADE"
        assert d.direction == "DOWN"
...
class TestDownBand:
    ...
    def test_at_upper_bound_0_18_fires(self):
        surface = _make_surface(probability_lgb_v9_5_eth=0.18)
        with _params():
            d = evaluate_v9_5_eth_blend_down_mid(surface)
        assert d.action == "TRADE"
        assert d.direction == "DOWN"
        assert d.entry_reason == "v9_5_eth_blend_down_mid_pass"

    def test_just_below_upper_bound_fires(self):
        surface = _make_surface(probability_lgb_v9_5_eth=0.17)
        with _params():
            d = evaluate_v9_5_eth_blend_down_mid(surface)
        assert d.action == "TRADE"
        assert d.direction == "DOWN"

    def test_at_lower_bound_0_12_skips(self):
        """Lower bound EXCLUSIVE — routes to down_high."""
    ...
    def test_in_down_high_band_skips(self):
        """0.08 is down_high's territory."""
...
class TestRuntimeOverride:
    def test_runtime_override_narrows_band(self):
        ...
        with _params():
            d = evaluate_v9_5_eth_blend_down_mid(surface)
        assert d.action == "TRADE"
        with _params(down_threshold=0.16):
            d = evaluate_v9_5_eth_blend_down_mid(surface)
        assert d.action == "SKIP"
    ...
    def test_runtime_override_widens_band(self):
        ...
        with _params():
            d = evaluate_v9_5_eth_blend_down_mid(surface)
        assert d.action == "SKIP"
        with _params(down_threshold=0.22):
            d = evaluate_v9_5_eth_blend_down_mid(surface)
        assert d.action == "TRADE"
        assert d.direction == "DOWN"

    def test_runtime_override_narrows_eval_band(self):
        surface = _make_surface(eval_offset=100, probability_lgb_v9_5_eth=0.15)
        with _params():
            d = evaluate_v9_5_eth_blend_down_mid(surface)
        assert d.action == "TRADE"
...
class TestMetadataShape:
    def test_metadata_contains_probability_on_trade(self):
        ...
        with _params():
            d = evaluate_v9_5_eth_blend_down_mid(surface)
        assert d.action == "TRADE"
        assert d.metadata["probability_lgb_v9_5_eth"] == pytest.approx(0.15)
        assert d.metadata["down_threshold"] == pytest.approx(0.18)
        assert d.metadata["down_threshold_min"] == pytest.approx(0.12)
        assert d.metadata["eval_offset_min"] == 60
        assert d.metadata["eval_offset_max"] == 240
        assert d.metadata["asset"] == "ETH"
        assert d.metadata["strategy_id"] == "v9_5_eth_blend_down_mid"
...
class TestConsecutiveTicks:
    ...
    def test_runtime_override_requires_two_ticks(self):
        ...
        with _params(min_consecutive_pass_ticks=2):
            d2 = evaluate_v9_5_eth_blend_down_mid(surface)
...
class TestCrossStrategyIndependence:
    def test_consec_state_isolated_from_down_low(self):
        ...
        assert down_mid._consec_state is not down_low._consec_state

        surface = _make_surface(probability_lgb_v9_5_eth=0.15)
        with _params():
            d = evaluate_v9_5_eth_blend_down_mid(surface)
        assert d.action == "TRADE"
        assert len(down_mid._consec_state) >= 1
    ...
    def test_consec_state_isolated_from_down_high(self):
        ...
        assert down_mid._consec_state is not down_high._consec_state
    ...
    def test_consec_state_isolated_from_up_mid(self):
        ...
        assert down_mid._consec_state is not up_mid._consec_state
    ...
    def test_consec_state_isolated_from_v9_5_eth_raw_lgb(self):
        ...
        assert down_mid._consec_state is not raw_lgb._consec_state
