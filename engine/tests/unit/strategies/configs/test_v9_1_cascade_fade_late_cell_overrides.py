"""Unit tests for v9_1_cascade_fade_late per-cell lgb_dist_min override (hub #402, PR #506).
...
Coverage:
  ...
  10. v9_1_model_not_loaded -> SKIP before any cell override logic
"""
...
_OFFSET_40 = 40  # sits in [24,60] AND resolves to T-31-60 band
_REGIME = "CASCADE"
...
class TestV91CascadeFadeLateCellOverrides:

    # 1. Cell relaxes DOWN floor
    def test_cell_relaxes_down_floor(self):
        from strategies.configs.v9_1_cascade_fade_late import evaluate_v9_1_cascade_fade_late
        token = _gp.set_active(_params(param_overrides_by_cell={_down_key(): {"lgb_dist_min_down": 0.0}}))
        try:
            d = evaluate_v9_1_cascade_fade_late(_make_surface())
        finally:
            _gp.reset_active(token)
        assert d.strategy_id == "v9_1_cascade_fade_late"
    ...
    def test_cell_tightens_down_floor(self):
        ...
        try:
            # dist=0.25 < 0.40 -> SKIP
            d = evaluate_v9_1_cascade_fade_late(_make_surface(probability_lgb_v9_1=0.25, probability_lgb=0.25))
        finally:
            _gp.reset_active(token)
        assert d.action == "SKIP"
        # v9_ensemble fallback (pc=None) uses v8 lgb_bucket gate name; direct path uses lgb_safety_floor
    ...
    def test_no_cell_match_uses_strategy_floor(self):
        from strategies.configs.v9_1_cascade_fade_late import evaluate_v9_1_cascade_fade_late
        # UP key with huge floor -- should not affect DOWN surface
        ...
        try:
            ...
            _gp.reset_active(token)
        assert d.strategy_id == "v9_1_cascade_fade_late"
        # dist=0.25 > default 0.10 -> no conviction-floor block
    ...
    def test_only_matching_key_fires(self):
        from strategies.configs.v9_1_cascade_fade_late import evaluate_v9_1_cascade_fade_late
        token = _gp.set_active(_params(param_overrides_by_cell={
            _down_key(): {"lgb_dist_min_down": 0.40},  # blocks DOWN
            _up_key(): {"lgb_dist_min_up": 0.0},       # irrelevant
        }))
        try:
            d = evaluate_v9_1_cascade_fade_late(_make_surface(probability_lgb_v9_1=0.25, probability_lgb=0.25))
        finally:
    ...
    def test_runtime_tunability(self):
        ...
        try:
            d1 = evaluate_v9_1_cascade_fade_late(_make_surface(probability_lgb_v9_1=0.25, probability_lgb=0.25))
        finally:
            _gp.reset_active(t1)
        ...
        try:
            d2 = evaluate_v9_1_cascade_fade_late(_make_surface(probability_lgb_v9_1=0.25, probability_lgb=0.25))
        finally:
            _gp.reset_active(t2)
        assert d2.skip_reason != "lgb_safety_floor", f"Relaxed should not block on conviction; got {d2.skip_reason!r}"
    ...
    def test_up_cell_override_relaxes_up_floor(self):
        ...
        try:
            d = evaluate_v9_1_cascade_fade_late(_make_surface(
                probability_lgb_v9_1=0.80, probability_lgb=0.80,
    ...
    # 10. v9_1_model_not_loaded SKIP before cell override
    def test_v9_1_model_not_loaded_skips_before_cell_override(self):
        from strategies.configs.v9_1_cascade_fade_late import evaluate_v9_1_cascade_fade_late
        token = _gp.set_active(_params(param_overrides_by_cell={_down_key(): {"lgb_dist_min_down": 0.0}}))
        try:
            d = evaluate_v9_1_cascade_fade_late(_make_surface(probability_lgb_v9_1=None))
        finally:
            _gp.reset_active(token)
        assert d.action == "SKIP"
        assert d.skip_reason == "v9_1_model_not_loaded"
        assert "cell_param_overrides_active" not in (d.metadata or {})
